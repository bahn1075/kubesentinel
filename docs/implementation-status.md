...
| Diagnosis Engine | `internal/diagnosis` | ✅ 동작 (심층분석) | **Rule Analyzer(결정론적 분류) · L1 상관분석+신뢰도 게이팅 · L2 client-go 근거 · L3 agentic 도구 루프+검증 패스**. tools 없으면 단발+검증 fallback. Runbook(메타데이터/키워드 검색)도 구현 |
