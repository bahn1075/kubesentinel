...
- **L3 agentic 도구 루프 + 검증**: LLM이 read-only 도구(`prometheus_query`/`loki_query`/`k8s_events`/`k8s_list_pods`/`k8s_get_nodes`/`image_inspect`)를 **프롬프트 기반 JSON 프로토콜**로 스스로 요청→백엔드 조회→재분석(최대 3회, 로컬 모델 호환 — 네이티브 tool-calling 불필요). 이후 **검증 패스**(회의적 리뷰어가 근거 대비 진단 비판·신뢰도 보정). `ToolRunner` 인터페이스는 diagnosis에 정의·collector가 구현·main이 주입(import cycle 회피). 도구 미가용(비 in-cluster) 시 단발+검증으로 graceful fallback.
