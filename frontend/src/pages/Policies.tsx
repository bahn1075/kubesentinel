...
export default function Policies() {
  ...
  if (loading || !policies) return <Skeleton title rows={6} />;
  ...
  return (
    <>
      ...
      {policies.length === 0 ? (
        <div className="empty">등록된 RemediationPolicy가 없습니다.</div>
      ) : (
        policies.map((p) => (
          <div className="section" key={p.name}>
            ...
            <div className="kv">
              ...
              <span className="mono">{Math.round(p.minConfidenceForPR * 100)}%</span>
            </div>
          </div>
        ))
      )}
